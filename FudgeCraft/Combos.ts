  ...
  export class Combos {
    ...
    private contains(_element: GridElement): boolean {
      for (let combo of this.found)
        for (let element of combo) if (element == _element) return true;
      return false;
    }
    ...
    private recurse(_element: GridElement, _combo: GridElement[]): void {
      ...
      for (let iMatch: number = matches.length - 1; iMatch >= 0; iMatch--) {
        ...
        if (iCombo >= 0) matches.splice(iMatch);
        else _combo.push(match);
      }

      for (let match of matches) this.recurse(match, _combo);
    }

    private findNeigborsOfSameColor(_element: GridElement): GridElement[] {
      let all: GridElement[] = grid.findNeigbors(
        _element.cube.cmpTransform.local.translation
      );
