namespace L02_FirstFudge {
  ...
  function update(_event: Event): void {
    ...
    if (isBallMoving) {
      if (
        keysPressed[f.KEYBOARD_CODE.ARROW_UP] &&
  ...
  function detectHit(_node: f.Node, _position: f.Vector3): boolean {
    let sclRect: f.Vector3 = _node.getComponent(f.ComponentMesh).pivot.scaling
      .copy;
